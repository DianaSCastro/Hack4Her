...
(function () {
  ...
  function extraerMeta() {
    ...
    return { url: window.location.href, dominio: window.location.hostname, titulo: document.title, timestamp: new Date().toISOString(), meta_tags: metas };
  }

  function extraerProductos() {
    const productos = [], vistos = new Set();
    const sels = ['[data-testid="product-title"]','[data-testid="allotment-product-tile"]','[class*="product-title"]','[class*="ProductTitle"]','[class*="product-card"]','[class*="ProductCard"]','[class*="product-name"]','[class*="item-name"]','[class*="sc-product"]','[class*="item-card"]','[class*="product-item"]','[class*="producto"]','article[class*="product"]','[data-product]','[itemprop="product"]','[itemtype*="Product"]'];
    sels.forEach(sel => {
      try { document.querySelectorAll(sel).forEach(el => {
        ...
        productos.push({ nombre: texto.split("\n")[0].trim().slice(0, 150), precio: precioMatch?.[0] || null, sku: skuMatch?.[0] || null, texto_completo: texto.slice(0, 400), selector: sel });
      }); } catch(e) {}
    });
  ...
  function extraerFormularios() {
    ...
    document.querySelectorAll("form, [role='form']").forEach(form => {
      ...
      form.querySelectorAll("input, select, textarea").forEach(el => {
        ...
        campos.push({ tag: el.tagName.toLowerCase(), tipo: el.type || "", nombre: el.name || el.id || "", label: getLabel(el), valor: el.value || "", placeholder: el.placeholder || "" });
      });
  ...
  function extraerTablas() {
    return [...document.querySelectorAll("table")].map(t => ({
      ...
      filas: [...t.querySelectorAll("tbody tr")].slice(0, 30).map(tr => [...tr.querySelectorAll("td")].map(td => td.innerText.trim())).filter(f => f.some(Boolean))
    })).filter(t => t.headers.length || t.filas.length);
  }

  function extraerTextos() {
    return [...document.querySelectorAll("h1,h2,h3,h4,p,span[class*='price'],span[class*='precio'],span[class*='name']")]
      .map(el => ({ tag: el.tagName, texto: el.innerText?.trim().slice(0, 200) || "" }))
      .filter(x => x.texto.length > 2).slice(0, 100);
  }
  ...
  function extraerLinks() {
    return [...document.querySelectorAll("a[href]")]
      ...
      .filter(l => l.texto.length > 1).slice(0, 50);
  }
